		...
		if(add.time.scale) {
			...
			plot.time.scale <- function(level, start.one, end.one, time.check=FALSE, grey.now=FALSE, cex.age.int) {
				...
				if(level == "Eon") {
					...
					if(label.timescale.names) text(total.length, mean(heights[start.one:end.one]), "Eon", pos=4, font=4, cex=cex.labels)
					if(time.check) {
						...
						if(label.timescale.names) text(total.length, Ntip(phy) + (Ntip(phy) / 35), "Ma", pos=4, font=4, cex=cex.labels, adj=c(0,1), xpd=TRUE)
					}
				}
			
				if(level == "Period") {
					plot.bar(bin.period, Period.colour, names(Period.colour),start.one,end.one, grey.line=grey.now, cex.labels.int=cex.labels)
					if(label.timescale.names) text(total.length, mean(heights[start.one:end.one]), "Period", pos=4, font=4, cex=cex.labels)
					if(time.check) {
						if(is.null(pos.age)) pos.age <- Ntip(phy) + (Ntip(phy) / 100)
				...
				if(level == "Epoch") {
					...
					if(label.timescale.names) text(total.length, mean(heights[start.one:end.one]), "Epoch", pos=4, font=4, cex=cex.labels)
					if(time.check) {
						if(is.null(pos.age)) pos.age <- Ntip(phy) + (Ntip(phy) / 100)
				...
				if(level == "Age") {
					...
					if(label.timescale.names) text(total.length, mean(heights[start.one:end.one]), "Age", pos=4, font=4, cex=cex.labels)
					if(time.check) {
						if(is.null(pos.age)) pos.age <- Ntip(phy) + (Ntip(phy) / 100)
