...
plotMCMCTree <- function(parameters, method=c("skewT", "skewNormal", "cauchy", "gamma", "bound"), title, upperTime, lowerTime=0, plotMCMCTreeData=FALSE) {
	...
	if(method == "skewT") {
		...
		if(plotMCMCTreeData == TRUE) {
			...
			density <- dst(time, xi=parameters[1], omega=parameters[2], alpha=parameters[3], nu=parameters[4])
	...
	if(method == "skewNormal") {	
		...
		if(plotMCMCTreeData == TRUE) {
			time <- seq(0, timeScale[2], 0.01)
			density <- dsn(time, xi=parameters[1], omega=parameters[2], alpha=parameters[3])
			return(cbind(time, density))			
			}
		}
	...
	if(method == "cauchy") {	
		...
		if(plotMCMCTreeData == TRUE) {
			out <- cauchyMCMCTree(xRange=timeScale, tL=parameters[1], p=parameters[2], c=parameters[3], minProb=parameters[4])
			return(out)
			}
	...
	if(method == "bound") {	
		...
		if(plotMCMCTreeData == TRUE) {
			out <- uniformMCMCTree(xRange=timeScale, tL=parameters[1], tU=parameters[2], minProb=parameters[3], maxProb=parameters[4])
			return(out)
			}
		}	
}