...
For example, for this example there are four calibrated nodes. The MCMCtree function ``tipDes`` can be used to recreate the monophyletic group list object. First the ``ape`` package is used to plot the tree and view the node label numbers. 
...
The code can be used to simultanaeouly estimate the parameter values that reflect the **a priori** time information for nodes and write files ready for MCMCtree input. MCMCtreeR can produce output files with the same type of distributions used to summarise **a prior** time information for all nodes, or different distributions can be used to reflect uncertainty on different internal nodes.
...
If we require different node calibration distributions on each calibrated nodes we can specify this by using the ``MCMCtreePhy`` function. Here there are different distributions applied to the internal nodes: a fixed root (node 1), skew normal (node 2), gamma (node 3), and upper distribution (node 4). For each input we give the associated parameter values in a vector in the order of nodes. i.e., for the ``minProb`` on four nodes can be set as 1, 2, 4 to be 1e-8 and node 3 to be 0.025
