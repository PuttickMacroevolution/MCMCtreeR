...
This guide demonstrates the ``MCMC.tree.plot`` function that plots time trees output from divergence time analysis using the molecular (or morphological) clock. ``MCMC.tree.plot`` includes options to plot the HPD age distribution for each node as node bars or the full distribution, and also includes arguments to plot a geological and/or absolute timescale.
